...
from agora.debate.cancel_policy import CancelPolicy
from agora.debate.metrics_writer import append_jsonl, write_debate_metrics
...
class DebateExecutor:
    ...
    def run(
        ...
        *,
        ...
        cancel_policy = CancelPolicy(cancel_check=cancel_check, cancel_after_round=cancel_after_round)
        ...
        async def _compute_round1() -> tuple[dict[str, str], list[dict[str, Any]]]:
            ...
            async def _one(role_id: str) -> tuple[str, str, dict[str, Any]]:
                async with sem:
                    cancel_policy.raise_if_requested(DebateCancelledError)
        ...
        try:
            ...
            cancel_policy.raise_if_round_done(1, DebateCancelledError)
            ...
            cancel_policy.raise_if_round_done(2, DebateCancelledError)
            ...
            for row in llm_call_rows:
                append_jsonl(session / "debate" / "llm_calls.jsonl", row)
